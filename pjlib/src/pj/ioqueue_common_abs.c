...
#   if PJ_HAS_TCP
	if (!pj_list_empty(&h->accept_list))
	{
		...
		rc=pj_sock_accept(h->fd, accept_op->accept_fd, accept_op->rmt_addr, accept_op->addrlen);
		if (rc==PJ_SUCCESS && accept_op->local_addr)
		{
			rc = pj_sock_getsockname(*accept_op->accept_fd, accept_op->local_addr, accept_op->addrlen);
		}
...
#   endif
		if (key_has_pending_read(h))
		{
			...
			else
			{
				...
				*/
#	    if defined(PJ_WIN32) && PJ_WIN32 != 0 || defined(PJ_WIN32_WINCE) && PJ_WIN32_WINCE != 0
				rc = pj_sock_recv(h->fd, read_op->buf, &bytes_read, read_op->flags);
...
#	    endif
	...
	PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT!=0
				/* Special treatment for dead UDP sockets here, see ticket #1107 */
				if (rc == PJ_STATUS_FROM_OS(ENOTCONN) && !IS_CLOSING(h) && h->fd_type==pj_SOCK_DGRAM())
				{
					replace_udp_sock(h);
				}
#endif

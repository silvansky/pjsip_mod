...
#if PJMEDIA_AUDIO_DEV_HAS_DUMMY
pjmedia_aud_dev_factory* pjmedia_dummy_factory(pj_pool_factory *pf);
#endif
...
#if PJMEDIA_AUDIO_DEV_HAS_DUMMY
	aud_subsys.drv[aud_subsys.drv_cnt++].create = &pjmedia_dummy_factory;
#endif
															...
															unsigned *p_local_index)
{
	...
	if (id < 0)
	{
		...
		for (i=0; i<aud_subsys.drv_cnt; ++i)
		{
			...
			else if (id==PJMEDIA_AUD_DEFAULT_CAPTURE_DEV && drv->rec_dev_idx >= 0) 
			{
				id = drv->rec_dev_idx;
				make_global_index(i, &id);
				break;
			}
			else if (id==PJMEDIA_AUD_DEFAULT_PLAYBACK_DEV && drv->play_dev_idx >= 0) 
			{
				id = drv->play_dev_idx;
				make_global_index(i, &id);
				break;

...
static pj_status_t callback_destroy(void *data)
{
	...
	if (strm->scr_tex) {
		CALLBACK_DestroyTexture(strm->scr_tex);
		strm->scr_tex = NULL;
	}
	if (strm->renderer) {
		CALLBACK_DestroyRenderer(strm->renderer);
		strm->renderer = NULL;
	}
...
//	{
//		CALLBACK_DestroyWindow(strm->window);
//	}
	strm->windowID = 0;
	...
	if (!strm->window) {
		...
		if (strm->param.flags & PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW_FLAGS) {
			if (!(strm->param.window_flags & PJMEDIA_VID_DEV_WND_BORDER))
				flags |= CALLBACK_WINDOW_BORDERLESS;
			if (strm->param.window_flags & PJMEDIA_VID_DEV_WND_RESIZABLE)
				flags |= CALLBACK_WINDOW_RESIZABLE;
		} else {
			flags |= CALLBACK_WINDOW_BORDERLESS;
		}
		...
		{
			flags |= CALLBACK_WINDOW_SHOWN;
		} else {
			flags &= ~CALLBACK_WINDOW_SHOWN;
			flags |= CALLBACK_WINDOW_HIDDEN;
		}
	}
			       ...
			       pjmedia_rect_size *new_disp_size)
{
	...
	if (strm->scr_tex) {
		...
		strm->dstrect.w = (Uint16)strm->param.disp_size.w;
		strm->dstrect.h = (Uint16)strm->param.disp_size.h;
		//CALLBACK_RenderSetViewport(strm->renderer, &strm->dstrect);
	}

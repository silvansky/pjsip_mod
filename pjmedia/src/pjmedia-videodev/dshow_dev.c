...
static pj_status_t dshow_factory_init(pjmedia_vid_dev_factory *f)
{
  HRESULT hr = mainInitHr = CoInitializeEx(NULL, COINIT_MULTITHREADED);
	//HRESULT hr = mainInitHr = CoInitializeEx(NULL, COINIT_APARTMENTTHREADED);
...
static pj_status_t dshow_factory_destroy(pjmedia_vid_dev_factory *f)
{
	...
	if(mainInitHr != RPC_E_CHANGED_MODE)
		CoUninitialize();
															...
															IBaseFilter **filter)
{
	...
	if (SUCCEEDED (hr))
	{
		...
		hr = MkParseDisplayName(pbc, df->dev_info[id].display_name, &pchEaten, &moniker);
		if (SUCCEEDED(hr))
		{
			hr = IMoniker_BindToObject(moniker, pbc, NULL, &IID_IBaseFilter, (LPVOID *)filter);
	...
	if (SUCCEEDED(hr))
	{
		...
		while (IEnumPins_Next(pEnum, 1, &pPin, NULL) == S_OK)
		{
			...
			if (dir == PJMEDIA_DIR_CAPTURE)
			{
				...
				hr = IPin_QueryInterface(pPin, &IID_IAMStreamConfig, (LPVOID *)&streamcaps);
				if (SUCCEEDED(hr))
				{
					...
					for (i = 0; i < icount; i++)
					{
						...
						hr = IAMStreamConfig_GetStreamCaps(streamcaps, i, &mediatype, (BYTE *)&vscc);
						if (FAILED (hr))
							continue;

						nformat = (dshow_fmt ? 1 : sizeof(dshow_fmts)/sizeof(dshow_fmts[0]));
						for (j = 0; j < nformat; j++)
						{
							...
							if (UuidCompare(&mediatype->subtype, (UUID*)dshow_format, &rpcstatus) == 0 && rpcstatus == RPC_S_OK &&
								  UuidCompare(&mediatype->formattype, (UUID*)&FORMAT_VideoInfo, &rpcstatus2) == 0 && rpcstatus2 == RPC_S_OK)
							{
								if (sup_fmt)
									sup_fmt[j] = PJ_TRUE;
								if (pSrcpin)
...
static pj_status_t dshow_factory_refresh(pjmedia_vid_dev_factory *f)
{
	...
	if (FAILED(hr) || ICreateDevEnum_CreateClassEnumerator(dev_enum, &CLSID_VideoInputDeviceCategory, &enum_cat, 0) != S_OK) 
	{
	...
	/* Add renderer device */
	dev_count += 1;
	df->dev_info = (struct dshow_dev_info*) pj_pool_calloc(df->dev_pool, dev_count, sizeof(struct dshow_dev_info));
	...
	if (dev_count > 1)
	{
		...
		while (IEnumMoniker_Next(enum_cat, 1, &moniker, &fetched) == S_OK)
		{
			...
			hr = IMoniker_BindToStorage(moniker, 0, 0, &IID_IPropertyBag, (void**)&prop_bag);
			if (SUCCEEDED(hr))
			{
				VARIANT var_name;
				...
				if (SUCCEEDED(hr) && var_name.bstrVal)
				{
					...
					hr = IMoniker_GetDisplayName(moniker, NULL, NULL, &wszDisplayName);
					if (hr == S_OK && wszDisplayName)
					{
						pj_memcpy(ddi->display_name, wszDisplayName, (wcslen(wszDisplayName)+1) * sizeof(WCHAR));
					...
					if (SUCCEEDED(hr))
					{
						...
						ddi->info.fmt_cnt = 0;
						for (j = 0; j < sizeof(dshow_fmts)/sizeof(dshow_fmts[0]); j++)
						{
							if (!sup_fmt[j])
								continue;
...
static void input_cb(void *user_data, IMediaSample *pMediaSample)
{
	...
	if (strm->cap_thread_initialized == 0 || !pj_thread_is_registered())
	{
		...
		status = pj_thread_register("ds_cap", strm->cap_thread_desc, &strm->cap_thread);
	...
	//pjmedia_video_format_detail *vfd1;
	...
	//strm->param.fmt.vid.size.w;
	//strm->param.fmt.vid.size.h;
																					...
																					const pjmedia_frame *frame)
{
	...
	hr = SourceFilter_Deliver(stream->dgraph.csource_filter, frame->buf, frame->size);
	if (FAILED(hr))
		return hr;
	...
	pjmedia_vid_dev_stream **p_vid_strm)
{
	...
	PJ_ASSERT_RETURN(param->dir == PJMEDIA_DIR_CAPTURE || param->dir == PJMEDIA_DIR_RENDER, PJ_EINVAL);
	...
	if (param->dir & PJMEDIA_DIR_CAPTURE)
	{
		...
		status = create_filter_graph(PJMEDIA_DIR_CAPTURE, param->cap_id, PJ_FALSE, PJ_FALSE, df, strm, &strm->dgraph);
		if (status != PJ_SUCCESS)
		{
			destroy_filter_graph(strm);
			/* Try to use default fps */
			PJ_LOG(4,(THIS_FILE, "Trying to open dshow dev with default fps"));
			status = create_filter_graph(PJMEDIA_DIR_CAPTURE, param->cap_id, PJ_FALSE, PJ_TRUE, df, strm, &strm->dgraph);
			...
			if (status != PJ_SUCCESS)
			{
				...
				PJ_LOG(4,(THIS_FILE, "Trying to open dshow dev with default "
					"size & fps"));
				status = create_filter_graph(PJMEDIA_DIR_CAPTURE, param->cap_id, PJ_TRUE, PJ_TRUE, df, strm, &strm->dgraph);
			}
	...
	else if (param->dir & PJMEDIA_DIR_RENDER)
	{
		...
		status = create_filter_graph(PJMEDIA_DIR_RENDER, param->rend_id, PJ_FALSE, PJ_FALSE, df, strm, &strm->dgraph);
		if (status != PJ_SUCCESS)
			goto on_error;
	}
	...
	{
		dshow_stream_set_cap(&strm->base, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW, &param->window);
	}
...
static pj_status_t dshow_stream_stop(pjmedia_vid_dev_stream *strm)
{
	...
	//if(stream->dgraph.media_filter != NULL)

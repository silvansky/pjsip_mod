...
struct pjmedia_vid_stream
{
	...
	// 
	pjmedia_vid_codec	    *codec_encoder;
	pjmedia_vid_codec	    *codec_decoder;
...
/* Prototypes */
static pj_status_t decode_frame(pjmedia_vid_stream *stream, pjmedia_frame *frame);
...
static pj_status_t stream_event_cb(pjmedia_event *event, void *user_data)
{
	...
	if (event->epub == stream->codec_encoder || event->epub == stream->codec_decoder)
	{
		/* This is codec event */
		switch (event->type)
		{
...
static pj_status_t put_frame(pjmedia_port *port, pjmedia_frame *frame)
{
	...
	*/
	...
	status = pjmedia_vid_codec_encode_begin(stream->codec_encoder, &enc_opt, frame,
		channel->buf_size -
	...
	for (;;)
	{
		...
		status = pjmedia_vid_codec_encode_more(stream->codec_encoder,
			channel->buf_size -
			sizeof(pjmedia_rtp_hdr),
			&frame_out,
			&has_more_data);
		if (status != PJ_SUCCESS)
		{
			LOGERR_((channel->port.info.name.ptr, "Codec encode_more() error", status));
			/* Ignore this error (?) */
			break;
		}
	}
...
static pj_status_t decode_frame(pjmedia_vid_stream *stream, pjmedia_frame *frame)
{
	...
	*/
	...
	for (cnt=0; ; ++cnt)
	{
		...
		pjmedia_jbuf_peek_frame(stream->jb, cnt, NULL, NULL, &ptype, NULL, &ts, &seq);
	...
	if (got_frame)
	{
		...
		if (status != PJ_SUCCESS)
		{
			LOGERR_((channel->port.info.name.ptr, "codec decode() error", status));
...
static pj_status_t get_frame(pjmedia_port *port, pjmedia_frame *frame)
{
	...
	*/
	if (stream->fmt_event.type != PJMEDIA_EVENT_NONE)
	{
		...
		//pjmedia_vid_codec_get_param(stream->codec, stream->info.codec_param); // ононб
		pjmedia_vid_codec_get_param(stream->codec_encoder, stream->info.codec_param);

		/* Update decoding channel port info */
		pjmedia_format_copy(&stream->dec->port.info.fmt, &stream->info.codec_param->dec_fmt);
	...
	{
		pjmedia_event_publish(NULL, port, &stream->miss_keyframe_event, PJMEDIA_EVENT_PUBLISH_POST_EVENT);
	...
	pjmedia_vid_stream **p_stream)
{
	...
	stream->name.slen = pj_ansi_snprintf(stream->name.ptr, M, "vstrm%p", stream);
	...
	if (!info->codec_param)
	{
		...
		status = pjmedia_vid_codec_mgr_get_default_param(stream->codec_mgr, &info->codec_info, &def_param);
		if (status != PJ_SUCCESS)
			return status;
	...
	vfd_enc = pjmedia_format_get_video_format_detail(&info->codec_param->enc_fmt, PJ_TRUE);
	vfd_dec = pjmedia_format_get_video_format_detail(&info->codec_param->dec_fmt, PJ_TRUE);
	...
	*p++ = '.'; *p++ = 'o'; *p++ = 'r'; *p++ = 'g';
	...
	//////////	return status;
	/* Init and open the codec. */
	status = pjmedia_vid_codec_init(stream->codec_encoder, pool);
	if (status != PJ_SUCCESS)
		return status;
	status = pjmedia_vid_codec_open(stream->codec_encoder, info->codec_param);
	if (status != PJ_SUCCESS)
		return status;
	...
	if (status != PJ_SUCCESS)
		return status;
	status = pjmedia_vid_codec_open(stream->codec_decoder, info->codec_param);
	if (status != PJ_SUCCESS)
		return status;
	...
	pjmedia_event_subscribe(NULL, &stream_event_cb, stream, stream->codec_encoder);
	pjmedia_event_subscribe(NULL, &stream_event_cb, stream, stream->codec_decoder);
	...
	*/
	...
	if (stream->frame_size == 0 || stream->frame_size > PJMEDIA_MAX_VIDEO_ENC_FRAME_SIZE)
	{
	...
	status = create_channel( pool, stream, PJMEDIA_DIR_ENCODING, info->tx_pt, info, &stream->enc);
	if (status != PJ_SUCCESS)
		return status;
	...
	*/
	...
	stream->rx_frames = pj_pool_calloc(pool, stream->rx_frame_cnt, sizeof(stream->rx_frames[0]));
	...
	/* Send RTCP SDES */
	len = create_rtcp_sdes(stream, (pj_uint8_t*)stream->enc->buf, stream->enc->buf_size);
	if (len != 0)
	{
		pjmedia_transport_send_rtcp(stream->transport, stream->enc->buf, len);
	}
	...
	stream->info.codec_param = pjmedia_vid_codec_param_clone(pool, info->codec_param);
...
PJ_DEF(pj_status_t) pjmedia_vid_stream_destroy( pjmedia_vid_stream *stream )
{
	...
	*/
	...
	////////////if (stream->codec)
	////////////{
	////////////	pjmedia_event_unsubscribe(NULL, &stream_event_cb, stream, stream->codec);
	////////////	pjmedia_vid_codec_close(stream->codec);
	////////////	pjmedia_vid_codec_mgr_dealloc_codec(stream->codec_mgr, stream->codec);
	////////////	stream->codec = NULL;
	////////////}

	if(stream->codec_encoder == stream->codec_decoder)
	{
		pjmedia_event_unsubscribe(NULL, &stream_event_cb, stream, stream->codec_encoder);
		pjmedia_vid_codec_close(stream->codec_encoder);
		pjmedia_vid_codec_mgr_dealloc_codec(stream->codec_mgr, stream->codec_encoder);
		stream->codec_encoder = NULL;
	}
	else
	{
		if (stream->codec_encoder)
		{
			pjmedia_event_unsubscribe(NULL, &stream_event_cb, stream, stream->codec_encoder);
			pjmedia_vid_codec_close(stream->codec_encoder);
			pjmedia_vid_codec_mgr_dealloc_codec(stream->codec_mgr, stream->codec_encoder);
		...
		if (stream->codec_decoder)
		{
			pjmedia_event_unsubscribe(NULL, &stream_event_cb, stream, stream->codec_decoder);
			pjmedia_vid_codec_close(stream->codec_decoder);
			pjmedia_vid_codec_mgr_dealloc_codec(stream->codec_mgr, stream->codec_decoder);
			stream->codec_decoder = NULL;
		}
	}

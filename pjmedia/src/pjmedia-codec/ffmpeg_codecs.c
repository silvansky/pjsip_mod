...
static pj_status_t h264_preopen(ffmpeg_private *ff)
{
	...
	if (ff->enc && ff->param.dir & PJMEDIA_DIR_ENCODING)
	{
		...
		if (profile && av_set_string3(ctx->priv_data, "profile", profile, 0, NULL))
		{
		...
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set H264 max NAL size to %d", ff->param.enc_mtu));
		}
		...
		//if (!av_set_int(ctx->priv_data, "threads", 0))
		//{
		//	PJ_LOG(3, (THIS_FILE, "Failed to set x264 threads"));
		//}
		...
		*/
		...
		////////}
		////////if (av_set_string3(ctx->priv_data, "tune", "animation+zerolatency", 0, NULL))
		////////{
		////////	PJ_LOG(3, (THIS_FILE, "Failed to set x264 tune 'zerolatency'"));
		////////}
		//if (av_set_string3(ctx->priv_data, "preset", "ultrafast", 0, NULL))
		if (av_set_string3(ctx->priv_data, "preset", "ultrafast", 0, NULL))
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set x264 preset 'ultrafast'"));
		}
		if (av_set_string3(ctx->priv_data, "tune", "zerolatency", 0, NULL))
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set x264 tune 'zerolatency'"));
		}
		...
		PJ_LOG(2, (THIS_FILE, "Start set params x264"));


		if (av_set_string3(ctx->priv_data, "partitions", "p4x4", 0, NULL))
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set x264 partitions"));
		}

		//if (!av_set_int(ctx->priv_data, "trellis", 0))
		if (av_set_int(ctx->priv_data, "direct-pred", "1"))
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set x264 direct-pred"));
		}
		if (av_set_int(ctx->priv_data, "wpredp", "0"))
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set x264 wpredp"));
		}
		//if (!av_set_int(ctx->priv_data, "weightp", 0))
		//{
		//	PJ_LOG(3, (THIS_FILE, "Failed to set x264 weightp"));
		//}
		if (!av_set_int(ctx->priv_data, "aq-mode", 0))
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set x264 aq-mode"));
		}
		if (!av_set_int(ctx->priv_data, "rc-lookahead", 0))
		{
			PJ_LOG(3, (THIS_FILE, "Failed to set x264 rc-lookahead"));
		}
		...
		PJ_LOG(2, (THIS_FILE, "End set params x264"));
...
static const ffmpeg_codec_desc* find_codec_desc_by_info(const pjmedia_vid_codec_info *info)
{
	int i;
																							...
																							pj_pool_factory *pf)
{
	...
	/* Create mutex. */
	status = pj_mutex_create_simple(pool, "ffmpeg codec factory", &ffmpeg_factory.mutex);
	if (status != PJ_SUCCESS)
		goto on_error;
...
#   define AVMEDIA_TYPE_VIDEO	CODEC_TYPE_VIDEO
		...
		*/
		...
		if ((c->encode && (desc->info.dir & PJMEDIA_DIR_ENCODING)) || (c->decode && (desc->info.dir & PJMEDIA_DIR_DECODING)))
		//if (( (c->encode || c->encode2) && (desc->info.dir & PJMEDIA_DIR_ENCODING)) || (c->decode && (desc->info.dir & PJMEDIA_DIR_DECODING))) // ПОПОВ: для x264 используется c->encode2
		{
			continue;
		}
		...
		//if (c->pix_fmts && (c->encode || c->encode2)) // ПОПОВ: для x264 используется c->encode2
		{
...
#if defined PJMEDIA_HAS_X264_CODEC && PJMEDIA_HAS_X264_CODEC != 0
		if( c->id == CODEC_ID_H264)
		{
			desc->enc = NULL;
		}
		else
		...
		/* Get ffmpeg encoder instance */
		if (c->encode && !desc->enc)
		//if ((c->encode || c->encode2) && !desc->enc) // ПОПОВ: для x264 используется c->encode2
		{
			desc->info.dir |= PJMEDIA_DIR_ENCODING;
			desc->enc = c;
		}
																		 ...
																		 pj_mutex_t *ff_mutex)
{
	...
	//if (ff->param.dir & PJMEDIA_DIR_ENCODING) // ПОПОВ
	if (ff->enc && ff->param.dir & PJMEDIA_DIR_ENCODING)
	{
		AVCodecContext *ctx = ff->enc_ctx;
		...
		*/
#if LIBAVCODEC_VER_AT_LEAST(52,113)  // ПОПОВ: убрал потому как в новом FFMpeg этого нет
		ctx->rc_lookahead = 0;
#endif
	}

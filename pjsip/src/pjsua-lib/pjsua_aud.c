...
pj_status_t pjsua_aud_subsys_init()
{
	...
	*/
	...
	// POPOV: we need only one speex band (wideband)
	codec_cfg.speex.option |= PJMEDIA_SPEEX_NO_NB;
	codec_cfg.speex.option |= PJMEDIA_SPEEX_NO_UWB;
...
PJ_DECL(pj_status_t) pjsua_aud_stream_pause_state (pjsua_call_id call_id, pjmedia_dir dir, pj_bool_t pause)
{
	...
	if(pause)
		status = pjmedia_stream_pause(audio_stream, dir);
	else
		status = pjmedia_stream_resume(audio_stream, dir);
